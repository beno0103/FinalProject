...
{
    //Safety check the state is idle, if it isnt, then a path is either already found
    ...
    //Cycle through and reset all the path flags to false
...
void PathFinder::update(double aDelta)
{
    ...
    //If we need our algorithym for debugging, change WHILE back to an IF
    while(isSearchingPath() == true && m_DestinationTileIndex != -1)
    {
        ...
        //Get the pathnode with the lowest F score from the open list, since this is sorted
        ...
        //Check to see if that node is at our destination tile
        int currentNodeTileIndex = m_Level->getTileIndexForTile(currentNode->getTile());
        if(currentNodeTileIndex == m_DestinationTileIndex)
        {
            //Build the final node path, this will use the current nodes parents to
            //track back through the path nodes all the way back to the start
            ...
            //If it is, we can stop searching and build out final path
        
        
        //If we got here that means we haven't reached our destination. We need to get the
        //adjacent tiles from the node and add them to the open list
        std::vector<Tile*> adjacentTiles;
        
            //Check the tile abovem is it walkable
        ...
        for(int i = 0; i < adjacentTiles.size(); i++)
        {
            ...
            else
            {
                ...
                //If the tile has a lower F score, update the G score and update the parent node
