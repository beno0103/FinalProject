...
void UISideMenu::setState(SideMenuState state)
{
    ...
    switch (m_State)
    {
        ...
        m_TargetX = m_X;
		//m_X = (m_Orientation == SideMenuRight ? -m_Width : ScreenManager::getInstance()->getScreenWidth());
        //m_TargetY = m_X;
        break;
        
        case SideMenuClosing:
        m_TargetX = (m_Orientation == SideMenuLeft ? -m_Width : ScreenManager::getInstance()->getScreenWidth());
		//m_TargetX = (m_Orientation == SideMenuRight ? -m_Width : ScreenManager::getInstance()->getScreenWidth());
        break;
        
        case SideMenuOpen:
        m_X = (m_Orientation == SideMenuLeft ? 0.0f : ScreenManager::getInstance()->getScreenWidth() - m_Width);
        m_TargetX = m_X;
		//m_X = (m_Orientation == SideMenuRight ? 0.0f : ScreenManager::getInstance()->getScreenWidth() - m_Width);
        //m_TargetX = m_X;
        break;
        
        case SideMenuOpening:
        m_TargetX = (m_Orientation == SideMenuLeft ? 0.0f : ScreenManager::getInstance()->getScreenWidth() - m_Width);
		//m_TargetX = (m_Orientation == SideMenuRight ? 0.0f : ScreenManager::getInstance()->getScreenWidth() - m_Width);
        break;
    }
}
