...
UIFont::~UIFont()
{
    //TODO: We will come back later to implement this
    if(m_Font != NULL)
    {
        delete m_Font;
        m_Font = NULL;
...
void UIFont::parseFontData(const char* fontName)
{
    ...
    if(inputStream.is_open() == true)
    {
        ...
        if(reader.parse(inputStream, root) == true)
        {
            ...
            {
                //Get the charcter and the frame data
    ...
    //Dont forget to close your input stream
    inputStream.close();
...
void UIFont::calculateSize()
{
    ...
    //Safety check the length of the text, make sure there is something to process
    if(m_Text.length() > 0)
    {
        for(int i = 0; i < m_Text.length(); i++)
        {
...
UIFont::UIFontFrame* UIFont::getFontFrame(std::string character)
{
    ...
    //If the font frme is NULL, try changing to upper case
    ...
    //There is still  change that fontFrame is NULL, so always safety
